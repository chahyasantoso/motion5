...
for (const [filePath, file] of Object.entries(report.files ?? {})) {
  ...
  for (const mutant of file.mutants ?? []) {
    if (mutant.status !== "NoCoverage")
      fileCounts[mutant.status] = (fileCounts[mutant.status] ?? 0) + 1;
  }
  ...
  files[filePath] = {
    total,
    killed,
    score: total === 0 ? 0 : Number(((killed / total) * 100).toFixed(2)),
    counts: fileCounts,
  };
  for (const [status, count] of Object.entries(fileCounts))
...
const threshold = Number(
  process.env.MUTATION_THRESHOLD ?? report.thresholds?.break ?? baseline.threshold,
);
const regressions = [];
for (const [filePath, expected] of Object.entries(baseline.files ?? {})) {
  const actual = files[filePath];
  if (actual && actual.score < expected.score)
    regressions.push({ filePath, expected: expected.score, actual: actual.score });
}
