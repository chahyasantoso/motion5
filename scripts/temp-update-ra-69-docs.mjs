...
tTrackHandle.setKeyframe(plugin, key, value) and removeKeyframe(plugin, key) are value-tier edits for a plugin group the node already authors. Both return the PatchBatch from one invalidation and never call replaceGraph, because a leaf carries no edge and the plugin is already in the chain by the bound-group precondition.
...
api = replaceOnce(api, "A `TrackHandle` carries `id`, `live`, `definition`, `requires`, `remove()`, `replace(next)`, `addObserve(observation)`, `removeObserve(observation)`, `setRequire(plugin, slot, source, memberKey?)`, `removeRequire(plugin, slot, memberKey?)`, `setKeyframeGroup(plugin, group)`, `removeKeyframeGroup(plugin)`, `setGoal(plugin, memberId, source)`, `removeGoal(plugin, memberId)`, `overrideValues(next)`, and `setValues(next)`.", "A `TrackHandle` carries `id`, `live`, `definition`, `requires`, `remove()`, `replace(next)`, `addObserve(observation)`, `removeObserve(observation)`, `setRequire(plugin, slot, source, memberKey?)`, `removeRequire(plugin, slot, memberKey?)`, `setKeyframeGroup(plugin, group)`, `removeKeyframeGroup(plugin)`, `setGoal(plugin, memberId, source)`, `removeGoal(plugin, memberId)`, `setKeyframe(plugin, key, value)`, `removeKeyframe(plugin, key)`, `overrideValues(next)`, and `setValues(next)`.", "api handle surface");
api = replaceOnce(api, "The six authoring members above are the structural tier, and each returns `void` because the return type carries the tier:", "The six structural authoring members above return `void` because the return type carries the tier:", "api structural count");
...
runtime = replaceOnce(runtime, "`setKeyframeGroup` and `removeKeyframeGroup` are the whole-group pair.", "`setKeyframeGroup` and `removeKeyframeGroup` are the whole-group pair.", "runtime anchor");
runtime = replaceOnce(runtime, "`setGoal` and `removeGoal` edit one entry of a solver's goals slot, addressed by the member id it is authored under, so two entries of one slot stay two edges.", "`setKeyframe` and `removeKeyframe` are the property-level pair. The named plugin group must already exist on the node. An existing key uses the live-value path, while a new key is added to the authored `values` section, validated, resolved, recompiled in place, and re-seeked at the existing progress. Removing an absent key is a no-op; removing the last leaf removes the empty section, group, and record shape. Both return one `PatchBatch`, never rebuild the graph, and refuse `keyframe-group-unbound` before writing.\n\n`setGoal` and `removeGoal` edit one entry of a solver's goals slot, addressed by the member id it is authored under, so two entries of one slot stay two edges.", "runtime property pair");
...
errors = replaceOnce(errors, "`setKeyframeGroup()`, `removeKeyframeGroup()`, `setGoal()`, `removeGoal()`, `overrideValues()`, and `setValues()`", "`setKeyframeGroup()`, `removeKeyframeGroup()`, `setGoal()`, `removeGoal()`, `setKeyframe()`, `removeKeyframe()`, `overrideValues()`, and `setValues()`", "errors stale surface");
