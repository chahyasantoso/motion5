...
async function main() {
  ...
  for (const [index, edit] of edits.entries()) {
    ...
    if (modes.length !== 1) {
      refuse(
        `${label} on \`${target}\` must name exactly one of find/replace, create, or delete, ` +
          `and it names ${modes.length}`,
      );
      continue;
    }
    const [mode] = modes;
    ...
    if (occurrences !== 1) {
      ...
      refuse(
        `${label} on \`${target}\`: the anchor matched ${occurrences} times and must match ` +
          `exactly once. It starts \`${first}\`.`,
      );
      continue;
    }
