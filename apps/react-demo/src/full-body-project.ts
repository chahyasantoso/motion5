...
/**
 * 13-Node Full Body Skeleton Rig Schema v5 Project.
 *
 * Correct Rotate-then-Translate Forward Kinematics (FK):
 * - 0deg = Right (+X), 90deg = Down (+Y), -90deg = Up (-Y)
 * - Pelvis: Root (x, y, rotation ~0deg)
 * - Spine: Chest (boneRot -90deg -> Points UP) -> Head (boneRot 0deg -> Continues UP)
 * - Arms: Upper (boneRot ~180deg from chest -> Points DOWN) -> Lower (boneRot ~30deg -> Elbow flex)
 * - Legs: Thigh (boneRot ~90deg from pelvis -> Points DOWN) -> Shin (boneRot ~15deg -> Knee flex) -> Foot (boneRot ~-90deg from shin -> Points FORWARD +X)
 */
