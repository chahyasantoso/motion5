...
The walking application synchronously commits its initial React root and acquires GSAP in a layout effect, so the producer's pin spacer exists before load/pageshow and native scroll restoration. Development runs wrap the root in React.StrictMode, exercising setup, teardown and setup again before the browser checks the surviving producer and pin spacer. This one bootstrap ordering boundary does not save or restore scroll itself, duplicate the trigger range, or flush Motion on each event. The application loads and mounts core nodes before subscribing its UI/threshold observer. The driver queues Motion progress; structural arm edits and the eventual scheduled pose are separate operations, not an atomic source/UI/publication transaction. The hysteresis remains insertion at progress at least 0.5 and removal below 0.45. Refresh alone must not cross those application thresholds. Cleanup attempts source observation removal, project disposal and clock disposal even if another release fails.
...
[The browser runbook](./SCROLL-BROWSER-SMOKE.md) separates actual browser observations from deterministic tests. Each Chromium result certifies only its named scenarios and exact tested revision. The shared-source/walking report does not by itself certify PR #351, history back/forward, other engines or manual interaction review; the reconciled IK consumer has its own report.
