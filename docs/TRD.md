...
- **Patch:** the frozen publication for one node at one revision, discriminated by its status: a value set when the node composed, a refusal when it did not, and identity alone once it has been destroyed.
...
- **TR-G-01 Qualify once.** Identity qualification happens exactly once, at load. No component downstream of load may observe an unqualified id. Implements FR-3.
- **TR-G-02 Total id functions.** Qualification and parsing must be pure, total, and lossless round-trips.
...
- **TR-G-05 Stable diagnostics.** Validation diagnostics must be sorted deterministically by rule id then path, so identical input yields byte-identical diagnostic output.
- **TR-G-06 Canonical order.** Order must be a pure function of qualified ids and authored order, with a deterministic tie-break. It must never depend on `Map` insertion order derived from runtime events. Implements I-12.
- **TR-G-07 Cycle reporting.** A cycle must be an error before mount, and its diagnostic must name the participating qualified ids as a minimal cycle path. Traversal must be iterative so that graph depth cannot overflow the stack.
- **TR-G-08 Live state identity.** Exactly one `ObservationState` exists per loaded project and its identity is stable across every successful and failed mutation. It must be mutated in place and must expose no rebuild-from-snapshot method. Implements FR-8, I-1, ADR-006.
...
- **TR-G-10 Transaction order.** A mutation must follow: build candidate, normalize and validate candidate, apply live-state changes with an undo journal, apply publisher schedule changes, commit the immutable snapshot, invalidate affected nodes, release the journal.
- **TR-G-11 Atomic failure.** Any failure before commit must replay the undo journal in reverse and rethrow the original error with candidate diagnostics attached. Graph IR, live edges, publisher indexes, lifecycle subscriptions, ownership, and published patches must be byte-identical to their pre-mutation snapshot. No partial application and no repair step. Implements I-2.
- **TR-G-12 Commit semantics.** Commit swaps the immutable IR snapshot. It must not recreate live state and must not recreate subscriptions.
- **TR-G-13 Total reference resolution.** A reference must resolve to a member or be recorded as a pending reference with a diagnostic. There is no third state, and a pending reference must never publish. Resolution must not depend on mount order. Implements FR-19.
...
- **TR-R-02 Seeded closure traversal.** Dirty seeds come from playhead invalidation and from mutations since the last flush. Traversal walks canonical order restricted to the downstream closure of those seeds.
- **TR-R-03 Compose at most once.** Each dirty node composes at most once per flush, memoized per batch, so a diamond composes its shared ancestor exactly once. Implements I-5.
...
- **TR-R-05 Patch shape.** A patch must be a union discriminated by `status`, whose four members are `ready`, `blocked`, `error`, and `destroyed`. Every member must carry `nodeId` and `revision`, and must declare only the payload its status owns: `ready` owns `values`, `sourceProgress`, `sourceRevisions`, and `diagnostics`; `blocked` and `error` own `diagnostics`; `destroyed` owns neither. A payload member declared at a status that cannot produce it is a defect rather than a formality, and a reader must narrow on `status` before reading one. `PublishInput` must be discriminated the same way, so no caller can ask for a pose at a status that owns none and no registry decides by hand which members survive a publication. A batch must carry its patches, tick number, seed set, and diagnostics summary. See ADR-098.
...
- **TR-R-08 Deduplication.** Equality is per variant. An unchanged status and unchanged inline diagnostics must produce no new revision and no subscriber notification, and for a `ready` patch the values, progress, and source revisions must be unchanged as well. A variant that owns no pose must not be compared on one. Implements FR-13.
...
- **TR-R-15 Composition root.** `Engine` constructs the runtime, wires adapters to ports, asserts each injected port, and performs no graph work itself.
...
- **TR-M-02 Deterministic stagger.** Stagger offsets must be a pure function of authored order and configuration, independent of mount timing.
- **TR-M-03 Track is a leaf.** Track owns playhead and progress state, interpolation inputs, resolved local plugin composition, local lifecycle, and renderer-neutral snapshots. It must have no children, no parent, no group host, and no composite playback. Implements FR-16.
- **TR-M-04 No traversal from Track.** No Track method may read or walk graph dependencies. Implements I-4.
- **TR-M-05 Plugin resolution.** Plugin registration must be explicit; duplicate registration of one name is an error rather than a silent overwrite; resolution happens once at track construction and yields a frozen ordered composition list; an unknown plugin is a load-time error diagnostic.
- **TR-M-06 Triggers.** Trigger types are `scroll`, `time`, and `manual`. A trigger is a delegate owned by Motion that converts an external signal into a playback command. No trigger in core may read the DOM. Attaching the same trigger twice is an error.
- **TR-M-07 Progress domain.** Progress must be clamped to `[0, 1]` and non-finite progress input must be rejected.
...
- **TR-P-02 Clock contract.** A clock exposes `subscribe(listener)` returning an unsubscribe function and emits frozen ticks carrying `tick`, `time`, and `delta`.
- **TR-P-03 Interpolator contract.** An interpolator exposes `create(config)` returning a timeline with `duration`, a progress getter and setter, and an idempotent `kill()`.
- **TR-P-04 Scheduler contract.** A scheduler exposes `schedule(job, options)` returning a cancel handle. Cancelling twice is safe and cancelling a completed job is a no-op.
- **TR-P-05 Fakes are first class.** Every port ships a fake, and the fake is the implementation the core test suite uses.
...
- **TR-L-02 Guard-before-notify.** The disposed guard flag must be set before any notification, never after, so reentrant teardown returns immediately.
- **TR-L-03 Owner-first order.** An owner removes graph membership, subscriptions, and edges before the contained object releases local resources, so no callback fires against a half-detached node.
- **TR-L-04 Fixed ownership.** Ownership is decided at construction or adoption and must never change during teardown. A borrowed runtime is detached by its borrower and destroyed only by its owner.
- **TR-L-05 Project-owned free tracks.** A free track authored in `freeTracks` is owned by the project and released with it.
- **TR-L-06 Flat retention.** Repeated load, mount, unmount, and dispose cycles must release subscriptions, timelines, graph membership, and cached patches, leaving retention flat across cycles.
- **TR-L-07 Adoption.** A free track adopted at runtime remains owned by its adopter; the project detaches rather than destroys it. Adopting a duplicate qualified id is an error, not a replacement.
- **TR-L-08 Churn safety.** Repeated unmount and remount cycles must leave no residual subscription and must produce correct blocked-then-ready transitions.

## 11. Diagnostics and error requirements

- **TR-E-01 One diagnostic shape.** Every diagnostic, at load or at runtime, must carry `ruleId`, `path`, `message`, `severity`, and optional `ids`. No second shape may exist.
...
- **TR-E-03 Candidate isolation.** A candidate project that produces an error must never replace an active project.
- **TR-E-04 Aggregate reporting.** Multiple runtime failures within one flush are reported as one aggregate error after the pass completes.
- **TR-E-05 Retry metadata.** Retry information is retained only for nodes whose publication failed and must be cleared on recovery. Retry scheduling keys off monotonic tick numbers.
...
- **TR-S-02 Documented surface.** Exported from `@motion5/core`: `Engine`, project loading, `Motion`, `Track`, plugin registration, triggers, patch subscription, the manual clock, port assertion helpers, and the authored contract constants.
- **TR-S-03 Unadvertised internals.** Internals are reachable only through `@motion5/core/internal`, which is not advertised and carries no stability promise. Deep wildcard imports must be blocked by the export map.
...
- **TR-PF-01 Committed budgets.** Budgets must cover graph traversal, dirty propagation, publication throughput, and memory retention, and must be versioned in `performance/budgets.json`.
- **TR-PF-02 Deterministic benchmarks.** Benchmarks must use fixed synthetic graphs, fixed seeds, and the manual clock. They must not depend on wall-clock timing for correctness.
- **TR-PF-03 Complexity ceilings.** A flush must be linear in the size of the dirty closure, not in the size of the graph. Canonical ordering must be recomputed on commit only, never per flush. A deliberately quadratic change must fail a budget.
- **TR-PF-04 Memory.** Repeated load, mount, unmount, and dispose cycles must show no monotonic growth in retained subscriptions, timelines, or cached patches.
- **TR-PF-05 Advisory has an expiry.** A benchmark that is advisory must record a removal date in session status. "Continue on error forever" is not a policy.
- **TR-PF-06 Promotion or deletion.** By the hardening phase every budget is required or its benchmark is deleted. No advisory job survives past its recorded expiry.

## 14. Supply chain and toolchain requirements

- **TR-SC-01 Reproducible install.** CI must install from a committed lockfile with `npm ci`, never an unconstrained install.
- **TR-SC-02 Pinned toolchain.** Node 24 is pinned in `engines` and in CI. Formatter and TypeScript versions are pinned exactly.
- **TR-SC-03 Least privilege.** CI runs with read-only repository permissions and must never rewrite contributor branches. Formatting is checked automatically and fixed only through a manually dispatched workflow. Implements ADR-007.
- **TR-SC-04 Concurrency.** CI cancels superseded runs per branch or pull request, and the same required matrix applies to pull requests and to protected branch pushes.
...
- **TR-T-02 Named invariant evidence.** Every architecture invariant must have a named executable test whose id matches the invariant id, and that test must fail when the invariant is broken.
- **TR-T-03 Determinism.** Tests must use the manual clock and must not read wall time, random values, animation frames, or unordered collection iteration.
...
- **TR-T-07 Documentation truth.** No document may claim a live gate or an implemented behavior that does not exist. Reality is reported in one status file only.
- **TR-T-08 Deletion discipline.** Code that exists only for a transition, and any document describing deleted code, must be removed in the same change that removes its reason to exist.
- **TR-T-09 Failure hygiene.** A flaky test is fixed or deleted in the same working session. Skipping a test to reach green is treated as a revert.
- **TR-T-10 Coverage is not a gate.** Coverage is reported but never the release gate. The release gate is the invariant matrix, the package consumer test, migration suite, contract suites, integration behavior, and deterministic benchmarks.
