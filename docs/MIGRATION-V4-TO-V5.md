...
- A `ScrollSource` still has its overshoot clamped for it, because a scroll position is a measured quantity, but pushing `NaN` or `Infinity` now throws `TypeError: ScrollSource progress must be finite.` instead of being forwarded.
