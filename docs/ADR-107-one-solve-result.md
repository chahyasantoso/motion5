...
**A non-finite goal is reported as the geometry it is, never laundered.** An infinite goal coordinate makes the goal a direction: the closed form aims along it fully extended and reports `too-far` with an infinite residual, and FABRIK straightens the path along it and reports its own kind for an unreachable goal with an infinite residual, never `too-far`. A `NaN` goal coordinate names no point or direction, so the solve refuses it by name before any result exists, rather than publishing `NaN` angles under a kind. [#489](https://github.com/chahyasantoso/motion5/issues/489) decided both and ADR-111's amendment of 2026-09-25 records them; `ik-goal-reading.ts` owns the reading for every strategy. `IR-9` pins the closed form's half. Adding an `invalid` kind was considered and rejected, because it would be a second validation owner for input `readFrame` already defines, and a kind nothing can produce from a loaded rig. The first draft of this record said every non-finite goal reads as `reached`; the independent quality pass on the pull request measured `+Infinity` reading as `too-far`, and the record was corrected to the behaviour rather than the behaviour to the record, because forcing `reached` would hide a direction the solve does state. Until #489 a `NaN` goal read as `reached` with a `NaN` residual beside `NaN` angles; that answer is withdrawn. A `NaN` extent, which is not a goal, still compares false against both bounds of `bandQuality` and reads as `reached` with a `NaN` residual, so `residual <= tolerance` stays false for it.
