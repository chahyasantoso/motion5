...
Red before green is executed and archived rather than described. Pull request [#139](https://github.com/chahyasantoso/motion5/pull/139) applies the separator suite alone to the unmodified parent `dba6cfd`. Run [32129021992](https://github.com/chahyasantoso/motion5/actions/runs/32129021992), archived at `logs/32129021992/` on `ci-logs`, reports `4 failed | 447 passed`, the four being exactly `E-1`, `E-2`, `E-3`, and `E-6`. The archived log carries the collision itself: `Duplicate observation edge "m/x|m|y/z|input||"`. `E-5` passes on that parent, which is what proves the duplicate check was green before the encoding changed.
