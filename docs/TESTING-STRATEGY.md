...
`stryker.config.json` holds an explicit `mutate` list rather than a directory glob, and that is deliberate: a glob would adopt every future module without anyone deciding to. What follows from it is that the list has to be maintained when logic moves, and [issue #419](https://github.com/chahyasantoso/motion5/issues/419) is what happens when it is not. The second quality pass over the runtime publication seam moved the interesting logic out of `graph-runtime.ts` into `graph-runtime-state.ts`, `diagnostic-report.ts` and `ports/scheduler.ts`, so every invariant that pass decided lived where no mutant was generated. That is not a gate failure, because the mutation run is not a required check. It is a coverage-honesty problem, because a score that did not move for want of an owner in the list reads exactly like a score that did not move because nothing regressed. Those paths were named explicitly, and `packages/core/test/unit/scripts/mutation-config.test.ts` is what keeps the list from drifting again silently. Phase B of [issue #443](https://github.com/chahyasantoso/motion5/issues/443) then moved the logic again and the list moved with it: `diagnostic-report.ts` was merged into `report.ts`, and the registry's lifecycle and the publication request left `patch-registry.ts` and `graph-runtime.ts` for `registry-phase.ts` and `publish-request.ts`, so those three are what the list names now. A path naming a file the tree no longer has generates no mutants and reports nothing about it, which is the same silence this policy exists to refuse rather than a second kind of problem. See ADR-094.
