...
The carried-forward pose is not silently deleted. A consumer that needs the last good pose while a node is blocked asks the registry for the last `ready` patch, because retention is the registry's question and not a field on a patch that is about something else. Which of the two that resolves to, a retained ready patch beside the current one or an accepted loss of the behaviour, is the one decision the first slice owes and the one this record deliberately leaves open, because it is a product question about what a blocked node renders as and it needs a failing-first case either way.
...
The work is cut so no slice exceeds the sizing rule in [PR-WORKFLOW.md](./PR-WORKFLOW.md), and the first slice is the one that decides the retention question with a failing-first case, because every later slice is mechanical once it is answered. Then the contract union and `PublishInput`, then `publish`, `samePatch` and `#notifyTerminal` in the registry, then the publisher and the runtime, then the React store and the DOM binding, then the suite. The owning pull request carries the plan, the measured semantic file count, and the exact tested SHA.
