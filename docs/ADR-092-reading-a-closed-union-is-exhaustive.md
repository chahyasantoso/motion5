...
The guarantee itself, in `packages/core/test/unit/lang/exhaustive.test.ts`: a switch that omits one arm fails to compile at the sink, with the accepting direction asserted in the same rig, and the throw covered for the cast that can still reach it.
