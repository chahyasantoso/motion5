...
- **`fk` Solver Binding**: A member bone binds `keyframes.fk.requires.solver` naming the solver
  node. It authors `values.length`, a pivot offset `x` and `y` if the rig wants one, and optionally
  `values.weight`, `values.minRotation`, `values.maxRotation`, and `values.influence`. The limits
  are static finite degrees in `[-180, 180]`, default missing bounds to `-180` and `180`, and are
  read by IK rather than FK. Both solves account for the pivot offset, so it is an ordinary authored
...
- **`fk.values.influence`**: a static finite number greater than `0` that weights this member's
  ...
  load has no registry with which to distinguish a bystander plugin key. FK claims the key for
  authoring but never reads it; IK reads it from the delivered member values. See ADR-110.
- **`fk.values.length`**: a finite number; a non-finite one reads as `0`. A negative `length` is accepted rather than refused and means a segment with no extent, in `fk`'s composition and in both solves alike, because one owner (`segmentExtent`) answers for all of them. It is a runtime meaning rather than a load rule because a live write, an animated stop or an easing overshoot can make a length negative after load. See ADR-107 and issue #482.
...
- **`fk3d.values`** (internal 3D prototype, not exported): `length` as for `fk`; `rotation`, `rotationX` and `rotationY`, the bone's local rest orientation relative to its parent in the CSS `Rz * Rx * Ry` convention, each defaulting to `0` with a non-finite angle reading as `0`; and `weight`, per member, with exactly `fk.values.weight`'s default, clamp and non-finite reading. With no solve the rest orientation composes and `weight` is never read; with one, `weight` blends the rest orientation toward the solved one along the shorter rotation arc, `0` and `1` being rest and solved exactly. Any orientation key on a solver-bound `fk3d` group with no `weight` beside it is `ik-solved-rotation-dead`. `x`, `y` and `z` are the pivot offset in the parent's rotated frame, each defaulting to `0` with a non-finite read as `0`; they are claimed by `fk3d` and composed before the local orientation. See ADR-117.
- **`ik3d.values.inspect`** (internal 3D prototype, not exported): a static boolean, default `false`, exactly as `ik.values.inspect`. `true` adds the 2D `inspection` record `{ kind, residual, iterations, atBound, residuals }` beside `rotations3d`; a two-bone chain's closed form reports only `reached`, `too-far`, `too-near` or `coincident`, with zero iterations and an empty `atBound`, and every other chain's 3D FABRIK reports `converged`, `iteration-cap`, `stalled` or `conflicted` with its pass count and an empty `atBound` (3D has no joint limits yet). `ik-inspect-malformed` and `ik-solver-key-misgrouped` apply unchanged. See ADR-120.
- **`ik3d.requires.pole`** (internal 3D prototype, not exported): optional. The world-space point the chain's elbow bends toward, bound on the solver node in the same `ik3d` group that binds `root`; its `x`, `y` and `z` are read as a goal's are, a non-finite coordinate reading as `0`. The bend plane is the plane through the effective root pivot, the goal and the pole, with the elbow on the pole's side. Unbound, or collinear with the pivot and the goal within `1e-9` of the pole's distance from the pivot, the ADR-114 root-local +z rule applies byte for byte. On a chain longer than two members or with branches, the pole bends the plane each root-to-leaf path is seeded in, which the iterative solve then leaves wherever the goals pull it (ADR-122). A pole under a group that bound no `root` is `ik-pole-without-chain`; under a plugin that declares no pole slot it is `plugin-unknown-requirement`. See ADR-118.
