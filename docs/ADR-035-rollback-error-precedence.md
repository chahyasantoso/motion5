...
`rejectAfterRollback` is no longer declared in `project-runtime.ts`. `rollback.ts` held it when issue #443 split that module up, and deleting `rollback.ts` moved it to `runtime/report.ts`, which is where the ownership sentence under **Decision** should now be read: one module-level owner in `runtime/report.ts`. "Used by both call sites" was true of the two this record decided and is not true of the tree: `rejectAfterRollback` has exactly one direct caller today, `run-plan.ts`, and `engine.ts` names it only in a comment. What both original call sites still reach is the policy, through the general form below. Nothing above it changed hands. The rollback set, the `errors` order, the verbatim message and the refusal to assign `cause` are all unchanged, and `P-1` through `P-3` still measure them from where they always did.
...
The adjacent hole **Consequences** recorded is closed, and a third caller has joined the two this record named. Issue #145 fixed `Engine`'s `destroyMotion` hook stopping before `motion.dispose()` when a trigger's `dispose` threw, in that hook's own ordering. What #469 fixed is this precedence at `ProjectRuntime`'s constructor, which released the composition it had been handed and then rethrew whatever came back, so a `GraphRuntime` construction failure was replaced by a host's release failure and the diagnosis was lost. It reaches this policy now. `P-4` and `P-5` in `packages/core/test/integration/rollback-error-precedence.test.ts` are the cases, beside `P-2`, whose shape they take: `P-4` is the aggregation and `P-5` is the bound that keeps it conditional rather than "wrap every failure", exactly as `P-3` holds that bound for the rollback path. They are `P-` rather than `D-` because the series split is by question and by owner, and which error a caller sees when a `ProjectRuntime` cleanup fails is this record's question. An earlier revision of this amendment named a `D-` case in the `Engine` teardown suite: that case drove the boundary through `Engine.load`, which cannot reach it, because `validateV5` refuses an unknown observation source before any composition exists to release.
