...
A `TrackHandle` carries `id`, `live`, `definition`, `requires`, `remove()`, `replace(next)`, `addObserve(observation)`, `removeObserve(observation)`, `overrideValues(next)`, and `setValues(next)`. A generic observation declares an output edge, which merges the source's contribution over the observer's composed patch. A dependency that feeds a track's own composition is bound under the plugin group's `requires` section, which is the only way a value enters composition.
