...
refuses the rest at load. That is one rule, and only the internal `ik3d` prototype declares a shape:

- `ik-chain-unsupported`, when a solver's plugin declares a narrower chain than the graph derived
  for it. The internal phase 8 `ik3d` solver declares exactly two members on one path from its root,
  so a one-member, three-member or branched `ik3d` rig is refused at load rather than erroring its
  solver on every tick. The 2D `ik` solver declares no narrower shape and never reports it. See
  ADR-114.
