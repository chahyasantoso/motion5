...
Constrained solving ([ADR-108](../ADR-108-constrained-2d-solving.md)) adds five load rules for
...
- `ik-bend-malformed`, when a solver's `bend` value, flat or grouped, is not the static string
  `"positive"` or `"negative"`.
- `ik-bend-conflicts-flip`, when one solver authors both `bend` and the legacy `flip` spelling, in any
  mix of flat and grouped. Use `bend` alone; `"positive"` is `flip: true` and bends the elbow toward
  increasing rotation, and `"negative"` is `flip: false`.
