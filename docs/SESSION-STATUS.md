...
- **Inspected main:** `0de48d3a91f59ac0a6e48c4322d9700cc7a7f030`, after the ownership mirror move [#363](https://github.com/chahyasantoso/motion5/pull/363). Exact CI and operation evidence belong to their PRs.
...
- **Read-budget headroom landed:** [#363](https://github.com/chahyasantoso/motion5/pull/363) is merged: `runtime/project-runtime.ts` moved thirty-four duplicated comment blocks into `runtime/project-runtime.md` and went from 57,567 to 47,208 bytes, so headroom against the 60,000 byte budget is 12,792 rather than 2,433. No behaviour, no public surface, no gate change. That headroom is where the convenience-API items on [#362](https://github.com/chahyasantoso/motion5/issues/362) land.
...
- **Unmerged citation ownership:** the second slice of [#362](https://github.com/chahyasantoso/motion5/issues/362) gates what the first one cleaned: a standalone comment in a mirrored source may not cite an `ADR-nnn`, an evidence id or an issue number, because a citation is provenance and the sister document owns it. The four pairs that still do carry a shrinking `CITATION_PENDING` entry rather than a waiver. `RB-19` through `RB-22` are its evidence. No behaviour, no public surface. Review, exact-head CI, and merge remain gates; no landed change is claimed.
