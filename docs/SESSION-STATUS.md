...
## Single-track mutation atomicity (issue #147)

Fixed by PR [#148](https://github.com/chahyasantoso/motion5/pull/148), closing issue [#147](https://github.com/chahyasantoso/motion5/issues/147). No new ADR, and none is claimed: ADR-031's `Failure ordering` already says single-track operations throw immediately, and this slice makes "immediately" mean "and changes nothing" on the one stage where it did not.

Found while verifying whether issue [#127](https://github.com/chahyasantoso/motion5/issues/127) was still a case. It was not, and it is closed: `addTrack` and `replaceTrack` resolve the compiled Track before they mutate, and `C-17` through `C-20` evidence it. What #127 never reached is the seeding call that ran after both writes. A resolvable Track can be disposed, because option C hands Track lifetime to the resolver's caller and `C-16` already shares one Track between two Motions, and `#timeline.progress` is injected `Interpolator` code either way. Both throw from the same line, so the rejection kept its mutation, and the next progress sweep then reported an id the caller had been told was refused.

Both methods now build the prospective entry list, seed against it, and commit it. Nothing is caught and undone, so the class gains no second notion of undo and ADR-035's precedence question does not arrive here: there is no committed state to roll back. The prospective list is load bearing rather than incidental, because `#effectiveProgress` reduces over the entries for `#totalDuration`. Seeding against the pre-mutation list would have seeded a staggered Motion with `0` instead of `0.25`, and `0.125` instead of `0.375`, so `#effectiveProgress` and `#totalDuration` now take the entry list as an explicit parameter and each call site says which one it means. Seeded values are byte-identical to the commit-first order on every successful path.

This is the first slice on this base to edit `packages/core/src/domain/motion.ts` since option C, so the `C-3` source guard is load bearing rather than incidental here too. It passes without being edited: no compiled Track is stored, and the `Track` import stays type-only.

## Documentation and package entrypoints

Landed from branch `docs/user-guide`. No behavior changes, and no test changes beyond what is stated below.

The repository had no user-facing documentation. All 56 files under `docs/` were implementor contracts, plans, audits, or decision records, so a consumer had nothing that answered "how do I drive this" without reading `engine.ts`. `docs/guide/` now holds six documents plus an index: getting started, triggers, rendering patches, runtime changes, errors and diagnostics, and an API reference grouped by entrypoint. The guide documents only what the `exports` map declares, and it defers to this file for what has landed rather than making its own status claims.

The root `README.md` was actively misleading. It described Phase 4 as reopened with `Engine.load()` publishing empty or input-only patches, Tracks rebuilt on every flush, and no keyframe compiler, which this file has contradicted for several slices. It now states what is shipped and points here for the authority.

`docs/FORMAT-WORKFLOW.md` is deleted and `docs/FORMATTING.md` absorbed it. Two documents owned one job and disagreed: the deleted one described a `.github/workflows/format-repair.yml` that does not exist, and archive files `run.log` and `jobs.json` that the archiver never wrote, which `docs/CI-WORKFLOW.md` already said never existed. The surviving document describes the read-only `format:check` gate inside `quality`, the write-enabled `format` job that runs only after behavioral green, and both manual workflows.

`packages/core/package.json` declares four more subpath entries: `./adapters/browser-clock`, `./plugins/transform`, `./plugins/fk`, and `./ports/fakes`. The decision record is ADR-036. `apps/react-demo` already imported all four, and they resolved only because both Vite configs alias `@motion5/core` to `packages/core/src` and bypass the `exports` map; a published install would have failed on every one of them. Plugins are the load-bearing case, because every authored keyframe key must be claimed by a registered plugin, so an unreachable `transformPlugin` means an unreachable `x` track. Nothing is added to `packages/core/src/index.ts`, so the boundary scanner's allow-list and the declaration-closure scan are both untouched.

Three gaps were found while writing the guide and are recorded there rather than fixed, because each needs its own decision. No `Scheduler` implementation ships, so every consumer writes one; `assertScheduler` is exported but no production scheduler is. The `Clock`, `ClockTick`, `Scheduler`, and `Cancel` types are not exported even though their assertions are, and neither is `EngineOptions`, so a consumer can satisfy the ports structurally but cannot name them in a signature. And neither package is published.
...
- Single-track mutation atomicity: PR [#148](https://github.com/chahyasantoso/motion5/pull/148), closing issue [#147](https://github.com/chahyasantoso/motion5/issues/147). Unit evidence is `packages/core/test/unit/domain/motion-track-mutation-atomicity.test.ts` cases `M-1` through `M-5`, with `M-6` as the seeded-value guard. `M-` is a new citation series, and the gate pattern in `packages/core/test/unit/scripts/evidence-case-ids.test.ts` was widened by exactly one prefix to admit it.
- Red evidence for the mutation atomicity is durable rather than claimed, and it is again the pull request's own first head: run [32148713472](https://github.com/chahyasantoso/motion5/actions/runs/32148713472), archived at `logs/32148713472/` on `ci-logs`, reports `5 failed | 470 passed` against the unmodified parent `43e250a`. The five are exactly `M-1` through `M-5`, and the same run proves `M-6` is green on that parent, which is what makes it a guard against the fix rather than evidence for it. `typecheck` and `format:check` passed in that same job, so the red is assertion-level rather than infrastructure. The archived log carries the committed ghost verbatim: `expected [Function] to throw error including 'Track is disposed.' but got 'Duplicate Motion track id: arm.'`
- Documentation and entrypoints: branch `docs/user-guide`, decision record ADR-036. No behavioral evidence is cited, because nothing behavioral changed. `governance-gates.test.ts` still asserts the `.` entry points at `./dist/index.d.ts` and `./dist/index.js`, and `public-declaration-surface.test.ts` still scans the closure of `src/index.ts`, so both cover the widened manifest without being edited.
...
- Two decision records share the number ADR-034: `ADR-034-edge-identity-order-and-label-separation.md` and `ADR-034-trigger-progress-range-ownership.md`. A citation of "ADR-034" is therefore ambiguous, and four comments in `packages/core/src/domain/motion.ts` plus one in `packages/core/src/adapters/scroll-trigger.ts` cite it for the progress range. Renumbering the progress-range record is its own slice, because it edits source comments and this one does not.
...
- Open, and not this slice's status to write: this file does not record PR [#146](https://github.com/chahyasantoso/motion5/pull/146) or the `D-` series it opened, and the two items immediately above are the ones that pull request closed. Flagged here rather than edited around, because one status file with a stale entry is still better than a second handoff note.
- `Track.setProgress` assigns `#progress` before it calls `#timeline.progress`, so a timeline that throws leaves the Track's own progress ahead of its timeline. Out of scope for #147, which owns `Motion`'s entry list rather than `Track`'s internals, and visible only to an injected interpolator that throws. It needs its own issue.
- No `Scheduler` implementation ships from any declared entrypoint, and the `Clock`, `ClockTick`, `Scheduler`, and `Cancel` types are not exported even though their assertions are. Both are recorded in `docs/guide/api-reference.md` and each needs its own decision rather than an incidental export.
...
- A single-track mutation resolves and seeds before it commits, so a refusal leaves nothing to undo.
...
- A document a consumer reads lives in `docs/guide/`, and it may only name a symbol or subpath the package `exports` map declares.
