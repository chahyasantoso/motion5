...
- **One premise of that plan is already stale, and it is stale in the plan's favour.** Its finding 7 reports `IncrementalGraphBuilder.#trackCache` as a live leak with no eviction path, and cites that as a second reason not to build a node-id-keyed cache. It was verified against a `main` that predates [ADR-058](./ADR-058-incremental-builder-cache-residency.md), which closed it for [issue #225](https://github.com/chahyasantoso/motion5/issues/225) with a per-build mark and sweep inside the adapter and evidence `EV-1` through `EV-8`. Eviction now has an owner, so that half of the argument is spent; finding 6, which is about `stageTrackDefinition` disposing a timeline the replacement just adopted, is what still settles the open decision on its own.
- What survives from finding 7 is the residency rule rather than the leak. After a build completes, the cache holds an entry for exactly the tracks that build walked, so one builder instance belongs to one project. `Engine.load` already constructs it that way, and sharing one across two projects thrashes to a permanent miss without affecting correctness.
- **Still open.** [Issue #176](https://github.com/chahyasantoso/motion5/issues/176) tracks transactional `replaceTrack` ordering after a failed recompile.
...
- An audit's claim about callers is a measurement, not a premise. A plan's claim about the code is the same: a finding verified against an older `main` is re-verified before it is cited, because a stale finding is how a decision gets made for a reason that no longer holds.
