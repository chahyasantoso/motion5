...
- Still open from section 8 of the trigger-driver plan: the `edgeKey` separator collision (8.2), owned by issue [#137](https://github.com/chahyasantoso/motion5/issues/137), and `seek` bypassing the Motion beyond documenting it (8.3). `T5` documents 8.3 and pins it with a test; it does not close it. 8.4 is closed by ADR-034.
- `docs/TRD.md` requirement `TR-M-07` still says progress "must be clamped to `[0, 1]` and non-finite progress input must be rejected" without saying by whom. After ADR-034 those are two jobs with two owners. Rewording a requirements document was left out of a bug fix and needs its own change.
