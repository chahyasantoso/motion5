...
**Phase:** runtime mutation model, trigger drivers through `T5`, compiled Track ownership, edge identity, trigger progress ownership, teardown ownership, single-track mutation atomicity, user documentation, and declared package entrypoints have landed.
...
- The React demo uses `TrackHandle` mutation and no longer calls the owner-based adoption wrappers.
...
- **Public port types:** consumers still cannot name `Clock`, `ClockTick`, `Scheduler`, `Cancel`, or `EngineOptions` from `@motion5/core`. Issue [#158](https://github.com/chahyasantoso/motion5/issues/158) will export them and add declaration-level consumer evidence.
- **Production scheduler:** `Engine` requires a Scheduler but no production implementation ships. Issue [#155](https://github.com/chahyasantoso/motion5/issues/155) will choose the scheduling semantics and expose an implementation from a declared entrypoint.
