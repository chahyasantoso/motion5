...
- Plugin-named authored keyframe groups landed, closing issue [#165](https://github.com/chahyasantoso/motion5/issues/165). `TrackDefinition.keyframes` now accepts `{ plugin: { leaf: { stops } } }`; `validateKeyframes` validates groups one level deep at authored paths, and `resolveForKeyframes` flattens grouped leaves back to unprefixed keys before compilation, so existing plugins compose unchanged. The colon separator is reserved across keyframe names and plugin metadata.
- Namespaced internal keys are stripped once before publish, closing issue [#166](https://github.com/chahyasantoso/motion5/issues/166). `Track.compose` removes `_`-prefixed and `:`-namespaced keys after the plugin chain and before the snapshot is frozen, so the publisher, `handle.get`, `subscribeNode`, and React all see the same filtered surface. `internalKeys` is deleted from the plugin contract and DOM adapter; cases `H-1` through `H-3` cover the strip and the React hook companion.
