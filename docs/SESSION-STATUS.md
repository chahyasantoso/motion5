...
- **#469's `PluginDefinition.stage` item is in review, and its first transport shipped a regression the follow-up checkpoint repairs:** `PluginStage` closes the stage vocabulary to `"prepare" | "compose"`, `VALID_STAGES` becomes a record keyed by that union so a third member owes an entry beside an arm, and `stageRank` reads both members through a `switch` ending at `unreachable(stage)` instead of testing one member and ranking every other value last. `cp006` also swapped the registration guard from `VALID_STAGES.has` to `Object.hasOwn`, which **moved the refusal set**: `Object.hasOwn` coerces its key, so `["prepare"]`, a boxed string and any object whose `toString` spells a member registered as members and the refusal arrived later as `Unhandled variant` from `stageRank`. `cp007` puts identity back behind one named reader, `isPluginStage`, measured against the retired set over twenty inputs with every message byte-identical, and pins the three coercing arrivals as cases. `cp006` also shipped a case that could not pass: it addressed the module-level `stageRank` with `member`'s class-member indent, so the helper refused the region; `cp007` names the column. An independent quality pass found both, and neither was caught by the four scanners, which is the honest limit of a slice prepared where `vitest` cannot run.
