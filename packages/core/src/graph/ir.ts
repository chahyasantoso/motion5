...
export function resolveObservationEdge(
  ...
  const edge: GraphEdge = Object.freeze({ observerId: observerNodeId, sourceId, role, ...(projection === undefined ? {} : { projection }) });
...
export function resolveRequirementEdge(
  ...
  const edge: GraphEdge = Object.freeze({ observerId: observerNodeId, sourceId, role: "input", requirement: Object.freeze({ plugin: binding.plugin, slot: binding.slot }) });
  return { edge, diagnostics: Object.freeze([]) };
}
