...
export function diag(ruleId: string, path: string, message: string, ids?: readonly string[]): Diagnostic {
...
export function qualifySource(source: string, motionId: string): string {
  ...
  if (source.includes("/"))
    return qualifyMotionTrack(source.slice(0, source.indexOf("/")), source.slice(source.indexOf("/") + 1)).value;
...
export function resolveObservationEdge(observation: ObservationDefinition, observerNodeId: string, ownerId: string, path: string): ResolvedObservation {
...
export function observationEdgeKey(observation: ObservationDefinition, observerId: string, ownerId: string): string {
...
export function collectTrack(track: TrackDefinition, owner: "motion" | "free", ownerId: string, authoredIndex: number, diagnostics: Diagnostic[]): GraphNode | undefined {
  try { assertAuthoredTrackId(track.id); }
  catch (error) {
