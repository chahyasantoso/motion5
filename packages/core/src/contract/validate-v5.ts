...
function usesThreeD(track: Record<string, unknown>): boolean {
  ...
  return (
    ...
    points.some((point) => isObject(point) && typeof point.z === "number" && point.z !== 0)
  );
}
...
function validateTrackShape(
  ...
  if (typeof id === "string" && id.length > 0) {
    if (seenIds.has(id))
      diagnostics.push(
        issue("track-duplicate-id", `${path}.id`, `Track id '${id}' is duplicated.`, "error", [id]),
      );
    seenIds.add(id);
  }
...
function validatePerspective(
  ...
  allTracks: readonly Record<string, unknown>[],
  diagnostics: Diagnostic[],
): void {
    ...
    if (source === targetId || source === `${targetId}`) {
      diagnostics.push(
        issue(
          ...
          `${edgePath}.source`,
          `Track '${targetId}' cannot observe itself.`,
          "error",
          [targetId],
        ),
      );
...
function hasCycle(
  ...
  for (const edge of edges) {
    ...
    if (outgoing.has(sourceNode) && outgoing.has(targetNode))
      outgoing.get(sourceNode)?.push(targetNode);
  }
...
export function validateV5(input: unknown): ValidationResult {
  ...
  for (const [index, rawMotion] of motions.entries()) {
      ...
      if (motionIds.has(motionId))
        diagnostics.push(
          issue(
            "motion-duplicate-id",
            `${path}.id`,
            `Motion id '${motionId}' is duplicated.`,
            "error",
            [motionId],
          ),
        );
    ...
    for (const [trackIndex, rawTrack] of rawMotion.tracks.entries()) {
      ...
      const track = rawTrack as unknown as Record<string, unknown>;
