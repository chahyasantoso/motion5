...
function addObservationDiagnostics(
  ...
  for (const [index, raw] of track.observes.entries()) {
    ...
    const qualifiedSource = source.startsWith("~/") || source.includes("/")
      ? source
      : localIds.has(source)
        ? `${ownerId}/${source}`
        : source;
    ...
    if (source === ownerId || qualifiedSource === `${ownerId}/${ownerId}` || qualifiedSource === ownerId) {
      diagnostics.push(issue("observation-self-reference", `${edgePath}.source`, `Track '${ownerId}' cannot observe itself.`, "error", [ownerId]));
    }
...
export function validateV5(input: unknown): ValidationResult {
  ...
  for (const record of motionRecords) {
    for (const track of record.tracks) {
      addObservationDiagnostics(track, `${record.path}.tracks[${record.tracks.indexOf(track)}]`, record.id, record.localIds, freeIds, qualifiedIds, edges, diagnostics);
    }
  }
