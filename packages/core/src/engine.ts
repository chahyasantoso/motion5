...
function assertValidProject(project: unknown): ProjectDefinition {
  ...
  if (!result.valid || result.value === null)
    throw new TypeError(result.diagnostics.length === 0 ? "Project failed v5 validation." : describeDiagnostics(result.diagnostics));
  return result.value;
}
export class Engine {
  ...
  load(project: ProjectDefinition): ProjectHandle {
    ...
    for (const motion of acceptedProject.motions) {
      ...
      for (const track of motion.tracks) nodes.set(qualifyMotionTrack(motion.id, track.id).value, { ...track, id: track.id });
    }
