import type { Diagnostic, Patch, PatchBatch, PatchListener, ProjectDefinition } from "./contract/v5";
...
export interface ProjectHandle { mount(nodeId: string, instance?: object): object; unmount(nodeId: string): void; seek(nodeId: string, progress: number): PatchBatch; subscribe(nodeId: string, listener: PatchListener): () => void; dispose(): void; }
...
function createHandle(runtime: RuntimeLike): ProjectHandle { return { mount: (nodeId, instance = {}) => runtime.mount(nodeId, instance), unmount: (nodeId) => runtime.unmount(nodeId), seek: (nodeId, progress) => runtime.seek(nodeId, progress), subscribe: (nodeId, listener) => runtime.graph.registry.subscribeNode(nodeId, listener), dispose: () => runtime.dispose() }; }
...
export class Engine {
  ...
  load(project: ProjectDefinition): ProjectHandle {
    const acceptedProject = assertValidProject(project); const tracks = new Map<string, Track>();
    const nodes = new Map<string, { id: string; duration?: number; keyframes?: Readonly<Record<string, unknown>> }>();
    for (const motion of acceptedProject.motions) for (const track of motion.tracks) nodes.set(`${motion.id}/${track.id}`, { ...track, id: track.id });
    for (const track of acceptedProject.freeTracks ?? []) nodes.set(`~/${track.id}`, { ...track, id: track.id });
    const compile = (nodeId: string): Track => {
      const existing = tracks.get(nodeId); if (existing) return existing;
      const definition = nodes.get(nodeId); if (!definition) throw new TypeError(`Unknown graph node "${nodeId}".`);
      const resolved = this.#plugins?.resolveForKeyframes(definition.keyframes ?? {}, `${nodeId}.keyframes`, { id: nodeId, duration: definition.duration });
      if (resolved?.diagnostics.some(({ severity }) => severity === "error")) throw new TypeError(describeDiagnostics(resolved.diagnostics));
      const track = new Track({ interpolator: this.#options.interpolator, interpolationConfig: definition, ...(resolved ? { plugins: resolved } : {}) });
      tracks.set(nodeId, track); return track;
    };
    try {
      ...
      const runtime = new ProjectRuntime(acceptedProject, { clock: this.#options.clock, scheduler: this.#options.scheduler, compose, setProgress: (nodeId, progress) => tracks.get(nodeId)!.setProgress(progress), disposeComposition: () => { for (const track of tracks.values()) track.dispose(); tracks.clear(); } });
      return createHandle(runtime);
    } catch (error) { for (const track of tracks.values()) track.dispose(); throw error; }
  }
}
