...
export class Engine {
  ...
  load(project: ProjectDefinition): ProjectHandle {
    ...
    for (const motion of acceptedProject.motions) for (const track of motion.tracks) nodes.set(qualifyMotionTrack(motion.id, track.id).value, { ...track, id: track.id });
    ...
    const compile = (nodeId: string): Track => {
      ...
      const resolved = this.#plugins?.resolveForKeyframes(definition.keyframes ?? {}, path, { id: nodeId, duration: definition.duration });
