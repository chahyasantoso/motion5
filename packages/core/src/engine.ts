...
function createHandle(runtime: RuntimeLike, signal: (motionId: string, signal: TriggerSignal) => void): ProjectHandle {
  ...
  Object.defineProperty(handle, "_runtime", { value: runtime, enumerable: false, writable: false, configurable: false });
  return handle;
}
...
export class Engine {
  ...
  load(project: ProjectDefinition): ProjectHandle {
    ...
    const buildMotion = (definition: MotionDefinition, entries: readonly MotionTrackEntry[]): Motion => {
      const created = triggerFactory.create({ motionId: definition.id, definition, clock: this.#options.clock, scheduler: this.#options.scheduler });
      createdTriggers.set(definition.id, created);
      let motion: Motion;
      try {
        motion = new Motion({ clock: this.#options.clock, scheduler: this.#options.scheduler, tracks: entries, trigger: created.port, disposeTracks: false, listenToClock: false, invalidate: () => { const currentIds = motion.tracks.map((t) => t.id); if (currentIds.length > 0) runtime.invalidate(currentIds); }, stagger: definition.stagger });
        motion.play();
        return motion;
      } catch (error) { createdTriggers.delete(definition.id); created.dispose(); throw error; }
    };
    try {
    ...
    } catch (error) { for (const motion of motions.values()) motion.dispose(); for (const trigger of createdTriggers.values()) trigger.dispose(); for (const track of tracks.values()) track.dispose(); throw error; }
  }
}
