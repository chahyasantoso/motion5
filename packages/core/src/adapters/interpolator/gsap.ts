...
function readDuration(config: unknown): number { if (!isRecord(config)) return 1; const duration = config.duration; return typeof duration === "number" && Number.isFinite(duration) && duration >= 0 ? duration : 1; }
function readTweenVars(config: unknown): Readonly<Record<string, unknown>> { return !isRecord(config) || !isRecord(config.tweenVars) ? {} : config.tweenVars; }
function readStops(value: unknown): readonly AuthoredStop[] { if (!isRecord(value) || !Array.isArray(value.stops)) return []; return value.stops.filter((stop): stop is AuthoredStop => isRecord(stop) && typeof stop.p === "number" && Number.isFinite(stop.p) && "v" in stop); }
...
export class KeyframeCompilationError extends TypeError { readonly diagnostics: readonly Diagnostic[]; constructor(diagnostics: readonly Diagnostic[]) { super(describeDiagnostics(diagnostics)); this.name = "keyframe-compilation"; this.diagnostics = diagnostics; } }
