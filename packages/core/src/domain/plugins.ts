...
export class PluginRegistry {
  ...
  register(plugin: PluginDefinition): void {
    assertName(plugin?.name); assertComposer(plugin?.compose); assertMetadata(plugin);
    if (this.#plugins.has(plugin.name)) throw new Error(`Plugin "${plugin.name}" is already registered.`);
    const detached = Object.freeze({ ...plugin,
    ...
    this.#plugins.set(plugin.name, detached); this.#orders.set(plugin.name, this.#registrationOrder++);
  }
  ...
  resolve(names: readonly string[], path = "plugins"): ResolvedPlugins {
    ...
    names.forEach((name, index) => {
      ...
      if (typeof name !== "string" || name.trim() === "") { diagnostics.push(diagnostic("plugin-name", itemPath, "Plugin name must be a non-empty string.")); return; }
      if (requested.has(name)) { diagnostics.push(diagnostic("plugin-duplicate-use", itemPath, `Plugin "${name}" is requested more than once.`, [name])); return; }
      requested.add(name); const plugin = this.#plugins.get(name);
      if (!plugin) { diagnostics.push(diagnostic("plugin-unknown", itemPath, `Plugin "${name}" is not registered.`, [name])); return; }
      plugins.push(plugin);
    });
  ...
  resolveForKeyframes(keyframes: Readonly<Record<string, unknown>>, path = "keyframes"): ResolvedPlugins {
    const plugins: PluginDefinition[] = []; const diagnostics: Diagnostic[] = []; const owners = new Map<string, string>();
    for (const key of Object.keys(keyframes)) {
      const matches = [...this.#plugins.values()].filter((plugin) => claims(plugin, key));
      if (matches.length === 0) { diagnostics.push(diagnostic("plugin-unknown-key", `${path}.${key}`, `No registered plugin claims authored key "${key}".`, [key])); continue; }
  ...
  #result(plugins: PluginDefinition[], diagnostics: Diagnostic[]): ResolvedPlugins { return Object.freeze({ plugins: Object.freeze(plugins), diagnostics: Object.freeze(diagnostics) }); }
}
