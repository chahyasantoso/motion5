...
A refusal and a release are two questions, and only the first may be answered inside a callback this class is still inside. `dispose()` owns the decision and sets the flag, so every member refuses and every handle answers stale from that line onward; this owns the release, and `#apply` may hold it back until the commit it is running has finished unwinding. Issue #312 extends that owner with one failure policy for direct disposal and deferred drains: attempt every independent release step, then record failures in the bounded diagnostics without throwing. No call-site flag or second error boundary decides whether an operation's outcome survives.
