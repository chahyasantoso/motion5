...
function fakeInterpolator(kill = vi.fn()): { interpolator: Interpolator; kill: ReturnType<typeof vi.fn> } {
  const timeline = { state: { x: 1 }, progress: vi.fn(), kill };
  return {
    kill,
    interpolator: { create: vi.fn(() => timeline) },
  } as unknown as { interpolator: Interpolator; kill: ReturnType<typeof vi.fn> };
}
...
describe("P2 runtime smell hardening", () => {
  it("preserves the last known good values when a node publishes error", () => {
    ...
    registry.publish({ nodeId: "source", sourceProgress: 0, status: "error", diagnostics: [{ ruleId: "test", path: "source", message: "boom", severity: "error" }] });
    registry.closeBatch();
  ...
  it("derives source revisions from the upstream patches consumed in the flush", () => {
    ...
    const consumer = node("consumer", [{ observerId: "consumer", sourceId: "source", role: "input" }], (inputs) => ({ values: inputs, sourceProgress: 0, sourceRevisions: {} }));
  ...
  it("reports a missing upstream instead of silently composing with an input hole", () => {
    const registry = new PatchRegistry();
    const publisher = new GraphPublisher(registry);
    const consumer = node("consumer", [{ observerId: "consumer", sourceId: "missing", role: "input" }], (inputs) => ({ values: inputs, sourceProgress: 0, sourceRevisions: {} }));
  ...
  it("chooses the blocked upstream deterministically by edge key, not authored edge order", () => {
    const registry = new PatchRegistry();
    const publisher = new GraphPublisher(registry);
    const failedA = node("a", [], () => { throw new Error("a failed"); });
    const failedB = node("b", [], () => { throw new Error("b failed"); });
    const consumer = node("consumer", [{ observerId: "consumer", sourceId: "b", role: "input" }, { observerId: "consumer", sourceId: "a", role: "input" }], (inputs) => ({ values: inputs, sourceProgress: 0, sourceRevisions: {} }));
  ...
  it("rejects host objects from interpolator state as a renderer-edge diagnostic, not a graph composition failure", () => {
    const { interpolator } = fakeInterpolator();
