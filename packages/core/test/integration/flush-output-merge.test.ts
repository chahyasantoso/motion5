...
describe("GraphPublisher output edges", () => {
  ...
  it("uses canonical edge order when output sources collide", () => {
    const registry = new PatchRegistry();
    const sourceA = node("a", 0, [], () => ({ values: { color: "a" }, sourceProgress: 0, sourceRevisions: {} }));
    const sourceB = node("b", 1, [], () => ({ values: { color: "b" }, sourceProgress: 0, sourceRevisions: {} }));
    const edges = [
      { observerId: "observer", sourceId: "b", role: "output" as const },
      { observerId: "observer", sourceId: "a", role: "output" as const },
    ];
    const observer = node("observer", 2, edges, () => ({ values: {}, sourceProgress: 0, sourceRevisions: {} }));
    const batch = new GraphPublisher(registry).flush(snapshot([sourceA, sourceB, observer]), ["a", "b"], 1);
    const expectedWinner = edgeKey(edges[0]!).localeCompare(edgeKey(edges[1]!)) > 0 ? "b" : "a";
    expect(batch.patches.find(({ nodeId }) => nodeId === "observer")?.values).toEqual({ color: expectedWinner });
  });
});
