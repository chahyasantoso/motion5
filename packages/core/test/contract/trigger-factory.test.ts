...
describe("trigger contract T1/T2", () => {
  ...
  it("validates every trigger row from the contract", () => {
    const invalid = [[{}, "trigger-shape"], [{ type: "unknown" }, "trigger-shape"], [{ type: "time", duration: 0 }, "trigger-time-duration"], [{ type: "scroll", source: "" }, "trigger-scroll-source"], [{ type: "scroll", source: 42 }, "trigger-scroll-source"]] as const;
    for (const [trigger, ruleId] of invalid) expect(validateMotionTrigger(trigger, "trigger").map((d) => d.ruleId)).toContain(ruleId);
  ...
  it("selects the intended T2 driver for each trigger type", () => {
    ...
    const expected = { manual: { acceptsExternalSignal: true, fed: false }, time: { acceptsExternalSignal: false, fed: true }, scroll: { acceptsExternalSignal: true, fed: false } } as const;
    for (const [type, want] of Object.entries(expected)) {
      const created = factory.create({ motionId: "scene", definition: { id: "scene", trigger: type === "time" ? { type, duration: 1000 } : { type }, tracks: [] }, clock: createManualClock(), scheduler: createFakeScheduler() });
