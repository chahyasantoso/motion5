 ...
 * appear extensionless, with a runtime extension (`./x.js` under NodeNext), or already as a
 * declaration path. Trying only `${specifier}.d.ts` meant every other spelling resolved to
 * nothing, and a scanner that ignores what it cannot resolve reports a clean boundary for a
 * closure it never walked. The caller treats an empty result as a failure, never as an edge
 * worth skipping.
 */
export function declarationCandidates(fromFile: string, specifier: string): string[] {
  const base = resolve(dirname(fromFile), specifier);
  const withoutExtension = (length: number) => base.slice(0, -length);
  if (specifier.endsWith(".d.ts") || specifier.endsWith(".d.mts") || specifier.endsWith(".d.cts"))
    return [base];
  if (specifier.endsWith(".js")) return [`${withoutExtension(3)}.d.ts`];
  if (specifier.endsWith(".mjs")) return [`${withoutExtension(4)}.d.mts`, `${withoutExtension(4)}.d.ts`];
  if (specifier.endsWith(".cjs")) return [`${withoutExtension(4)}.d.cts`, `${withoutExtension(4)}.d.ts`];
  return [`${base}.d.ts`, `${base}.d.mts`, `${base}.d.cts`, join(base, "index.d.ts")];
}

describe("public declaration surface (P1-9)", () => {
  it(
    "scans the emitted entry declaration closure, not just source strings",
    async () => {
      ...
      try {
        ...
        // otherwise report an empty forbidden list and look like a passing boundary.
  ...
  it("resolves every declaration specifier shape and leaves unknown targets unresolved", () => {
    ...
    const emitted = new Set([
      entry,
      join(emitRoot, "contract", "v5.d.ts"),
      join(emitRoot, "ports", "index.d.ts"),
    ]);
    const pick = (specifier: string) =>
      declarationCandidates(entry, specifier).find((candidate) => emitted.has(candidate));

    expect(pick("./contract/v5")).toBe(join(emitRoot, "contract", "v5.d.ts"));
    expect(pick("./contract/v5.js")).toBe(join(emitRoot, "contract", "v5.d.ts"));
    expect(pick("./contract/v5.d.ts")).toBe(join(emitRoot, "contract", "v5.d.ts"));
