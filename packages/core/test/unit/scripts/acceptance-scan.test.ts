...
async function createFixture() {
  ...
  await writeFile(join(docs, "TRD.md"), "# fixture\n");
  return root;
}
...
describe("acceptance mapping scan (D3)", () => {
  ...
  it("reports duplicate acceptance ids", async () => {
    const root = await createFixture();
    try {
      await writeFile(
        join(root, "docs", "acceptance-map.json"),
        JSON.stringify({
          version: 1,
          items: [
            { id: "same", test: "test/a.test.ts" },
            { id: "same", test: "test/b.test.ts" },
          ],
        }),
      );
      await expect(scanAcceptance(root)).resolves.toEqual([
        "same: missing test/a.test.ts",
        "same: duplicate id",
        "same: missing test/b.test.ts",
      ]);
    } finally {
      await rm(root, { recursive: true, force: true });
    }
