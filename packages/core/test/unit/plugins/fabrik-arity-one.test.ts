...
// deleted it because the dispatcher owns arity now. `FB-10` pins that a one-member chain loads,
// which is a question for `buildGraphIR`; what actually changed at that arity is what the runtime
...
//
// A one-member chain is a real solve: the bone points at its goal. That is the honest answer at
// every distance, because a single segment has one degree of freedom and no length to spend, so the
// only question a solve can answer for it is which way it faces. Zero was not a degenerate answer,
// it was a different pose.
//
// The direct fixture is a structural `SolveMember`; authored `values` are not involved because this
// case exercises the dispatcher rather than the member-reading adapter.
...
describe("a one-member chain is a solve rather than a fallback (Slice D4)", () => {
  it("FB-16 a single bone faces its goal at every distance instead of publishing zero", () => {
    ...
    // than left to the tolerance above: a fallback returning zero for a goal on the `+x` axis
    // would pass a position assertion and mean nothing.
    ...
    //    pose, so deleting the fallback did not quietly widen what a solve accepts.
    expect(() => solveChain(ROOT, chain, false)).toThrow(/at least one goal/);
  });
});
