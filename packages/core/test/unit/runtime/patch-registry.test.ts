...
describe("PatchRegistry", () => {
  ...
  it("I-8 suppresses unchanged republish and advances revisions only on change", () => {
    const registry = new PatchRegistry();
    registry.beginBatch(1, ["hero/arm"]);
    const first = registry.publish({ nodeId: "hero/arm", values: { x: 1 }, sourceProgress: 0, status: "ready" });
    registry.closeBatch();
    registry.beginBatch(2, ["hero/arm"]);
    const unchanged = registry.publish({ nodeId: "hero/arm", values: { x: 1 }, sourceProgress: 0, status: "ready" });
    const second = registry.publish({ nodeId: "hero/arm", values: { x: 2 }, sourceProgress: 0, status: "ready" });
    registry.closeBatch();
