...
| B1 | Prepare-stage plugin contribution | Red test recorded | `fix/B1-track-contribution` | [track](../packages/core/src/domain/track.ts), [plugins](../packages/core/src/domain/plugins.ts), [test](../packages/core/test/unit/domain/track-contribution.test.ts) | red [`9f2390f9`](https://github.com/chahyasantoso/motion5/commit/9f2390f9a1ce992f35c938ccf7526ba08e51c085); implementation next |
...
Implement B1 on `fix/B1-track-contribution`: prepare-stage plugins must contribute to interpolation config once during Track construction, before the interpolator timeline is created; composition must not rerun contribution. Then run CI on the branch and record green evidence before starting B2.
