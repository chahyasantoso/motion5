...
One request at a time. The workflow applies the single `.json` file it finds here, and refuses when it finds two rather than choosing one for you. A refused request stays where it is: correct that same file and push it again.
